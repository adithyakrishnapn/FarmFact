...
// Weather route to get hourly weather data with 3-hour intervals
$app->get('/weather', function (Request $request, Response $response) {
    ...
    $panchayat = !empty($queryParams['panchayat']) ? trim($queryParams['panchayat']) : null;
    $district = !empty($queryParams['district']) ? trim($queryParams['district']) : null;
    $state = !empty($queryParams['state']) ? trim($queryParams['state']) : null;
    
    // Get optional days parameter (1-10 days, default is 1 for today only)
    $days = isset($queryParams['days']) ? (int)$queryParams['days'] : 1;
    
    // Validate required parameters
    if (!$panchayat || !$district || !$state) {
        $response->getBody()->write(json_encode([
            'error' => 'Missing required parameters. Please provide panchayat, district, and state.'
        ]));
        return $response
            ->withStatus(400)
            ->withHeader('Content-Type', 'application/json');
    ...
    try {
        ...
        // Debug logging
        error_log("Weather request for location: " . $location . " for " . $days . " days");
        ...
        $params = [
            'key' => $weatherApiKey,
            'q' => $location,
            'days' => $days, // Use the days parameter
        ...
        error_log("Error in weather route: " . $e->getMessage());
